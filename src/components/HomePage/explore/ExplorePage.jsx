...
const FlameIcon = ({ className }) => (
    <svg className={className} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M12 2C10.97 6.5 6 9.5 6 14C6 17.31 8.69 20 12 20C15.31 20 18 17.31 18 14C18 9.5 13.03 6.5 12 2Z" fill="currentColor"/>
        <path d="M12 20C10.34 20 9 18.66 9 17C9 15 12 12 12 12C12 12 15 15 15 17C15 18.66 13.66 20 12 20Z" fill="var(--bg-card)" opacity="0.6"/>
    </svg>
);
            ...
            <div className="explore-hero-footer">
                <div className="explore-hero-author">
                    ...
                    <div className="user-name-container">
                        <p className="user-newsfeed-name">{journal?.users?.name || "Unknown author"}</p>
                ...
                <div className="explore-hero-stats">
                    <div className="explore-hero-score-pill">
                        <FlameIcon className="explore-hero-score-flame" />
                        {hotScore}
                    </div>
...
const RunnerCard = ({ journal, index, hotScore, onClick }) => {
    ...
    >
        <div className="explore-runner-rank-col">
            <span className={`explore-runner-rank ${index <= 2 ? "explore-runner-rank--top" : ""}`}>
                {index + 1}
            </span>
        </div>

        <div className="explore-runner-body">
            ...
            <div className="explore-runner-meta">
                ...
                <div className="explore-runner-stats">
                    <div className="explore-score-pill">
                        <FlameIcon className="explore-runner-flame-icon" />
                        {hotScore}
                    </div>
                    <div className="reading-time-container">
                        <p className="reading-time-text">{journal?.reading_time ? `${journal.reading_time} ${journal.reading_time > 1 ? 'mins' : 'min'} read` : CalculateText(previewText)}</p>
...
const ExplorePage = () => {
    ...
    const {
        ...
        queryKey: ["explore-search", userId, committedSearchQuery],
    ...
    const heroJournal = !isSearchMode && journals.length > 0 ? journals[0] : null;
    const runnerJournals = !isSearchMode && journals.length > 1 ? journals.slice(1) : isSearchMode ? journals : [];
        ...
        <div className="explore-page-container">
                ...
                <div className="search-top-bar">
                    ...
                    {isSearchMode ? (
                        <span className="search-mode-pill">
                            {isFetchingSearch ? "Searching..." : "Matched"}
                        </span>
                    ) : (
                        <span className="search-mode-pill">
            ...
            {runnerJournals.length > 0 && (
                <div className={`explore-runners-section ${isSearchMode ? "explore-runners-section--search" : ""}`}>
                    {!isSearchMode && (
                        <div className="explore-section-title explore-section-title--runners">
                            Runners up
                        </div>
                    )}
                        ...
                        {runnerJournals.map((journal, i) => {
                            ...
                            return isSearchMode ? (
                                <motion.div
                                    key={journal.id}
                                    className="cards"
                                    onClick={() => handleClickPost(journal.id)}
                                    role="button"
                                    tabIndex={0}
                                    onKeyDown={(e) => {
                                        if(e.key === "Enter" || e.key === " "){
                                            e.preventDefault();
                                            handleClickPost(journal.id);
                                        }
                                    }}
                                    initial={{ opacity: 0, y: 14 }}
                                    animate={{ opacity: 1, y: 0 }}
                                ...
                                >
                                    ...
                                    <div className="card-icons-container">
                                        ...
                                        <div className="explore-meta-right">
                                            <div className="explore-score-pill">
                                                Hot score: {hotScore}
                                            </div>
                                            <div className="reading-time-container">
