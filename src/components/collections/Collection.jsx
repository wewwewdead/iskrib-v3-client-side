...
const CharCounter = ({ current, max }) => {
    ...
    const cls = pct >= 90 ? 'char-counter-fill--danger' : pct >= 70 ? 'char-counter-fill--warning' : 'char-counter-fill--normal';
    return (
        <div className="char-counter-container">
            <div className="char-counter-bar">
                <div className={`char-counter-fill ${cls}`} style={{ width: `${Math.min(pct, 100)}%` }} />
            </div>
            <span className="char-counter-text">{current}/{max}</span>
        </div>
    );
};
...
const Collections = () =>{
    ...
    return(
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" width="28px" height="28px" viewBox="0 0 24 24" version="1.1">
                        <title>ic_fluent_book_formula_recent_24_filled</title>
                            ...
                            <g id="🔍-System-Icons" stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
                                <g id="ic_fluent_book_formula_recent_24_filled" fill="currentColor" fillRule="nonzero">
        ...
        <div className='create-collection-container'>
            ...
            <div onClick={(e) =>{handleClickShowCollectionEditor(e)}} className='btn-collection btn-collection--secondary' role="button" tabIndex={0} aria-label="Create a collection" onKeyDown={(e) => { if(e.key === 'Enter' || e.key === ' ') handleClickShowCollectionEditor(e) }}>
                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" width="24px" height="24px" viewBox="0 0 24 24" id="add-collection" data-name="Flat Color" className="icon flat-color">
                    <g id="SVGRepo_bgCarrier" strokeWidth="0"/>
                    <g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"/>
                    <g id="SVGRepo_iconCarrier">
                        <path id="primary" d="M20,22H7a1,1,0,0,1,0-2H20V5a1,1,0,0,1,2,0V20A2,2,0,0,1,20,22ZM16,2H4A2,2,0,0,0,2,4V16a2,2,0,0,0,2,2H16a2,2,0,0,0,2-2V4A2,2,0,0,0,16,2Z" style={{fill: 'currentColor'}}/>
                        <path id="secondary" d="M10,14a1,1,0,0,1-1-1V11H7A1,1,0,0,1,7,9H9V7a1,1,0,0,1,2,0V9h2a1,1,0,0,1,0,2H11v2A1,1,0,0,1,10,14Z" style={{fill: 'var(--bg-primary)'}}/>
                    </g>
                </svg>
                Create a collection
            </div>
        ...
        {showCollectionEditor && (
            ...
            animate={{opacity:1, scale:1, transition: {type: "tween", duration: 0.15}}}
            exit={{opacity:0, scale:0.95, transition: {type: "tween", duration: 0.1}}}
            ...
            >
                ...
                <div className='collection-input-name'>
                    ...
                    <div className='input-name-container'>
                        ...
                        <CharCounter current={collectionTitle.length} max={50} />
                    </div>
                </div>
                <div className='collection-input-description'>
                    ...
                    <div className='input-description-container'>
                        ...
                        <CharCounter current={collectionDescription.length} max={550} />
                    </div>

                    <div className='create-or-add-container'>
                        <div onClick={(e) => handleClickShowOlderPost(e)} className='btn-collection btn-collection--primary' role="button" tabIndex={0} aria-label="Select journals" onKeyDown={(e) => { if(e.key === 'Enter' || e.key === ' ') handleClickShowOlderPost(e) }}>
                            Select journals
                        </div>
                    </div>
                </div>
                    ...
                    {journals?.map((journal) => {
                        ...
                        return(
                            <div className='collection-cards-selected-old-post' key={journal?.id}>
                                <div className='journal-title text-truncate'>
                                    {journal?.title}
                                </div>
                                <div className='journal-text text-truncate-2'>
                ...
                <button disabled={isDisabled} onClick={(e) => handleClickSaveCollection(e, journalIds)} className='btn-collection btn-collection--primary' style={{margin: '1rem 0 1rem 1rem'}}>
                    Save Collection
                </button>
